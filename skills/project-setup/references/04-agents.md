...
**Orchestrator — proactive alignment duty (bake this into the generated def verbatim in spirit):** the orchestrator DETECTS roster/CLAUDE.md/memory needs itself, in every session, during normal work — the user never has to notice or re-run project-setup. Watch for signals continuously, not only at plan boundaries: new language/platform surface appearing (build file, native dir, new client shell), an agent repeatedly failing or working outside its scope lines, a wave touching a stack no gate command covers, the same convention correction arriving twice, an agent def contradicting how work actually flows, stale/missing agent memory. Any signal → propose the delta NOW — new agent / scope edit / retire / CLAUDE.md change / memory update — as a concrete diff with a one-line reason; user says yes → apply + commit in the same session (rule 9 gate always: approval before write, every autonomy mode; no → record in STATE.md `## Notes`). The `align`/`agents` subcommands are manual shortcuts, not the mechanism — the mechanism is this standing duty.
