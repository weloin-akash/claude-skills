...
| `align` (alias `realign`) | 02 §1 boundary map + §5 | On-demand alignment for an already-adopted project: rebuild the boundary map (languages × platforms × toolchains, CI coverage), then the 02 §5 pass — roster vs boundaries, gates vs stacks (missing toolchain / CI-never-builds), board vs git, CLAUDE.md deltas. Every agent-file + CLAUDE.md diff user-approved (rule 9); deferred findings → STATE.md `## Notes`. Run after adding a platform/language outside the normal plan flow |
| `plan` | 05 | Plan next phase now (macro-plan ask included) |
| `upgrade` | 04 §2 + templates | Bring an adopted project up to this skill's CURRENT conventions. Baseline = the `<!-- project-setup: vN -->` stamp in the project's CLAUDE.md (templates.md; absent = pre-stamp project, diff everything, warn that customization vs staleness can't be auto-told apart — user judges each hunk). Diff generated artifacts (CLAUDE.md blocks, agent defs, gate/report/contract templates, `scripts/run-gates.sh`) vs current templates; missing-feature list DERIVED from templates.md section headings, never hardcoded → table current/changed/missing; user picks; apply via 04 §2 (rule 9 gates every agent-file + CLAUDE.md diff), update the stamp. Never touches code; record applied deltas in STATE.md `## Notes` |
