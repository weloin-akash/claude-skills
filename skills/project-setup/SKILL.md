...
13. **Roster tracks boundaries — re-derived as the project grows, never frozen at setup.** The roster is a function of the DETECTED tech boundaries (language × platform × toolchain: e.g. Rust core / Go server / TS client / Android / iOS / desktop shell), not of the project type named at init. **Boundary-drift check** runs at every plan completion, phase transition, and ALIGN adoption (02 §Initial alignment): scan actual stacks (build files, native dirs, CI targets) vs the roster's scope lines + gate commands. New boundary appeared (single-platform → cross-platform, new language, new client shell) or one died → STOP and propose a roster delta — build new agent / edit scope + conventions of existing / retire — plus the CLAUDE.md, contract-doc, and gate updates the new stack needs; **user approves via the update-aware diff (04 §2), then apply — never silently, never deferred**. Companion: a wave's gates must COVER every stack the wave touched — a stack with no gate command, or whose toolchain is absent on the host, is a FAIL for that wave (06 §5b); CI that never builds one of the project's platforms is a boundary-drift finding, not background noise.
14. **Agent-board sync (Blink wired).** What the agent works from and what the board shows must not diverge: every status the agent acts on (started/done/blocked) hits `.blink/` in the same commit (rule 7), and every RESUME re-reads the board — `blink:sync` (installed) or fallback compare of `.blink/` statuses vs progress files + `git log`; disagreement → reconcile to git truth FIRST, commit, then continue (same protocol as the progress-file check, 06 §Resume). The board is agent-consumed state (`blocked_by`/`paused_reason` gate dispatch; `decisions/` stop re-litigating settled choices), not a user-only display.
...
- Creating, editing, or retiring a `.claude/agents/*.md` file without explicit user approval (rule 9 — every autonomy mode)
