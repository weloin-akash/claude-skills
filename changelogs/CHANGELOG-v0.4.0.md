...
- **Ops rules (06)**: multi-session (pull-before-wave, yield-to-origin, pathspec commits in a shared root checkout, cwd-relative memory sweep), worktree traps (origin-default base → reset to local tip; commit-same-run or auto-clean loses work; ancestor-checked cleanup; exit codes not grep), crash salvage (resume dead subagents, commit surviving worktree work in place).
- **Gate runner shipped** (`assets/run-gates.sh` → `scripts/run-gates.sh` at gates setup): per-gate evidence files, missing toolchain = FAIL, red exit on any failure — 06 §5b mechanized. Optional SessionStart hook (04 §5) prints STATE.md every session.
- **`upgrade` subcommand**: diffs an adopted project's generated artifacts vs the skill's current templates + lists missing newer features; every delta user-approved.
- **Orchestrator proactive alignment duty (04 §4)**: the generated orchestrator itself detects roster/CLAUDE.md/memory needs continuously during normal work — signals: new language/platform surface, out-of-scope agent failures, uncovered gate stacks, repeated convention corrections, stale memory — and proposes the concrete diff immediately; user approves, it applies same session. The user never has to notice drift or re-run project-setup; `align`/`agents` are manual shortcuts only.
...
- **Rule 9 hardened — agent definitions always user-gated**: any create/edit/retire of `.claude/agents/*.md` AND any CLAUDE.md change, from any trigger, requires explicit user approval of the concrete diff BEFORE the write, in every autonomy mode (AUTO never waives it). Applies to agent files, roster shape and CLAUDE.md only — normal code/doc edits follow the autonomy mode.
...
- **ALIGN handles git-less codebases**: no repo / empty history → user-approved `git init` + baseline commit (adoption needs a diff base; decline = stop), git findings marked n/a, commit style + branching asked instead of detected.
- **ALIGN gains an initial-alignment pass (02 §5)** — and a new `align` (`realign`) subcommand runs the same pass on demand for an already-adopted project (e.g. right after adding a platform/language): on every existing-project adoption — roster vs boundary map, gates vs stacks (missing toolchain / CI-never-builds reported), board vs git reality — each delta user-approved before applying.
